$(function(){
    ...
    const newmessage = $(`
      <div class="message-wrapper" data-id=${message.id}>
        <div class="sender-profile">
          <img class="profile-avatar" src="http://res.cloudinary.com/dlzzsx5ex/image/upload/${message.sender_photo_path}" >
        </div>
  ...
  function update(){ //この関数では以下のことを行う
    if($('.message-wrapper')[0]){ //もし'messages'というクラスがあったら
      var message_id = $('.message-wrapper:last').data('id'); //一番最後にある'messages'というクラスの'id'というデータ属性を取得し、'message_id'という変数に代入
    } else { //ない場合は
